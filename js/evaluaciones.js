...
class EvaluacionUI {
    constructor() {
        ...
        this.criterios = [];
        this.evaluacion = {};
        this.eventListenersAdded = new Set();
    ...
    cacheDOM() {
        ...
        this.guardarBtn = document.querySelector('.fa-save');
        this.reporteBtn = document.querySelector('.fa-download');
    }
    ...
    async init() {
        try {
            ...
            // For now, we assume all criteria are for all evaluations. This can be filtered if needed.
    ...
    render() {
        ...
        document.getElementById('saveButton').addEventListener('click', () => this.saveEvaluation());
    }
    ...
    renderCriterios() {
        this.criteriosContainer.innerHTML = ''; // Limpiar el contenedor
            ...
            <div class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="bg-green-500 h-2 rounded-full" id="criterio-${criterio.id}-progress-bar" style="width: 0%"></div>
                    </div>
                </div>
                <div class="space-y-2 mb-4" id="criterio-${criterio.id}-doc-list">
                    <!-- Documentos cargados se insertan aquí -->
                </div>
                ...
                <div class="mt-4">
                    ...
                    <input type="number" id="criterio-${criterio.id}-score-input"
    ...
    addEventListenersToCriterio(criterio) {
        ...
        dropArea.addEventListener('drop', (e) => {
            ...
            if (files.length > 0) {
                this.uploadFile(criterio.id, files[0]);
            }
        });
        fileInput.addEventListener('change', () => {
            if (fileInput.files.length > 0) {
                this.uploadFile(criterio.id, fileInput.files[0]);
            }
        });
    }
    ...
    createDocumentItem(criterioId, doc) {
        ...
        div.innerHTML = `
            <div class="flex items-center space-x-2">
                ...
                <span class="text-sm">${doc.nombre_archivo}</span>
            </div>
            <button class="text-red-500 hover:text-red-700 delete-doc" data-criterio-id="${criterioId}" data-doc-name="${doc.nombre_archivo}">
    ...
    async uploadFile(criterioId, file) {
        ...
        try {
            ...
            // Re-add label event listener if it was removed
            const fileInput = document.getElementById(`criterio-${criterioId}-file-input`);
    ...
    async handleDeleteDocument(event) {
        ...
        if (!confirm(`¿Estás seguro de que quieres eliminar el documento "${docName}"?`)) return;

        try {
            // We need to get the storage path to delete the file.
            // The new `eliminarDocumento` function handles this.
            await this.supabase.eliminarDocumento(this.proveedorId, `criterio-${criterioId}`);
    ...
    updateCriterionUI(criterioId) {
        const criterioData = this.evaluacion.criterios[criterioId] || { score: 0 };
        const score = criterioData.score;
    ...
    updateTotalScore() {
        ...
        const totalScore = Object.values(this.evaluacion.criterios).reduce((acc, curr) => acc + curr.score, 0);
    ...
    async saveEvaluation(isManual = true) {
        try {
            ...
            const evaluacionData = {
                proveedor_id: this.proveedorId,
                tipo_evaluacion: 'GENERAL', // Hardcoded as per new design
